...
public class CinemaService {
    ...
    public Cinema remake(Cinema remakeCinema) {
        var currentCinema = cinemaRepository
                .findById(remakeCinema.getId())
    ...
    public Cinema delete(String name) {
        var delCinema = cinemaRepository
                .findByNameOfCinema(name)
                .orElseThrow(() -> new RuntimeException("Cinema request is not in database !"));
        roomRepository.findAll().forEach(x -> {
            if (x.getCinema().getNameOfCinema().equals(name)) x.setCinema(null);
        });
