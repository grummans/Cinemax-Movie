...
public class UserService {
    ...
    public String registerUser(UserRequest request) {
        ...
        user.setRole(
                roleRepository.findByRoleName("ROLE_USER").orElseThrow(() -> new RuntimeException("Role Not Found")));
        user.setUserStatus(userStatusRepository
                .findUserStatusByCode("False")
                .orElseThrow(() -> new RuntimeException("Data Not Found")));
        user.setRankCustomer(rankCustomerRepository
                .findRankNameByPoint(20)
                .orElseThrow(() -> new RuntimeException("Data Not Found")));
    ...
    public void logout(LogoutRequest request) throws ParseException, JOSEException {
        ...
        InvalidateToken invalidateToken =
                InvalidateToken.builder().id(tid).expiryTime(expTime).build();
    ...
    public MessageResponse changeRoleUser(UpdateRoleRequest request) {
        ...
        user.setRole(roleRepository
                .findByRoleName(formatRole(request.getRole()))
                .orElseThrow(() -> new ApplicationException(ErrorCode.DATA_NOT_FOUND)));
    ...
    private String formatRole(String role) {
        switch (role) {
            case "admin":
                role = roleRepository
                        .findByRoleName("ROLE_" + role.toUpperCase())
                        .orElseThrow(() -> new ApplicationException(ErrorCode.DATA_NOT_FOUND))
                        .getRoleName();
                break;
            case "moderator":
    ...
    public MessageResponse updateUser(UpdateUserRequest request) {
        ...
        var current =
                userRepository.findById(request.getId()).orElseThrow(() -> new RuntimeException("Data not found !"));
        if (request.getPoint() == 0) request.setPoint(current.getPoint());
        var exists = userRepository.findAll().stream()
                .anyMatch(x -> x.getUsername().equals(request.getUserName())
    ...
    public UserResponse getUserById(int id) {
        return entityConverter.convertEntityToDTO(
                userRepository.findById(id).orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND)));
    }
