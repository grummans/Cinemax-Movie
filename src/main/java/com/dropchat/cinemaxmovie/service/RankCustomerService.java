...
public class RankCustomerService {
    ...
    public RankCustomer remake(RankCustomer remakeRequest) {
        var current =
                rankRepo.findById(remakeRequest.getId()).orElseThrow(() -> new RuntimeException("Data not found !"));
    ...
    public RankCustomer delete(String name) {
        var current = rankRepo.findByName(name).orElseThrow(() -> new RuntimeException("Data not found !"));
