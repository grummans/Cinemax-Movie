...
public class TicketService {
    ...
    public Ticket remake(Ticket remakeTicket) {
        var current = ticketRepository
                .findById(remakeTicket.getId())
                .orElseThrow(() -> new RuntimeException("Date not found !"));
        if ((int) remakeTicket.getPriceTicket() == 0) remakeTicket.setPriceTicket(current.getPriceTicket());
    ...
    public Ticket delete(String code) {
        var current = ticketRepository.findByCode(code).orElseThrow(() -> new RuntimeException("Date not found !"));
        billTicketRepository.findAll().forEach(x -> {
            if (x.getTicket().getCode().equals(code)) billTicketRepository.delete(x);
        });
